...
void SHA512_Update(const void *buffer, size_t len, SHA512_CTX *ctx);
...
void *
sha512_read_ctx (const SHA512_CTX *ctx, void *resbuf)
{
  int i;
  char *r = (char*)resbuf;
...
static void
sha512_conclude_ctx (SHA512_CTX *ctx)
{
...
void *
SHA512_Final (SHA512_CTX *ctx, void *resbuf)
{
...
/* Compute SHA512 message digest for LEN bytes beginning at BUFFER.  The
   ...
   digest.  */
void *
SHA512(uint8_t *buffer, size_t len, void *resblock)
{
  SHA512_CTX ctx;
  ...
  SHA512_Update (buffer, len, &ctx);

  /* Put result in desired memory area.  */
  return SHA512_Final (&ctx, resblock);
}

void
SHA512_Update (const void *buffer, size_t len, SHA512_CTX *ctx)
{
...
# define UNALIGNED_P(p) ((uintptr_t) (p) % __alignof (uint64_t) != 0)
...
{
  uint64_t const *words = (uint64_t const *)buffer;
...
#define R(A, B, C, D, E, F, G, H, K, M)                                   \
  do                                                                      \
    {                                                                     \
      uint64_t t0 = u64plus (SS0 (A), F2 (A, B, C));                           \
      uint64_t t1 =                                                            \
