...
// MotoPoW
unsigned int GetSerializeSize(const MotoPoW& m, int nType, int nVersion);
template<typename Stream> void Serialize(Stream& os, const MotoPoW& m, int nType, int nVersion);
template<typename Stream> void Unserialize(Stream& is, MotoPoW& m, int nType, int nVersion);
...
// MotoPoW
inline unsigned int GetSerializeSize(const MotoPoW& m, int nType, int nVersion)
{
    return
        GetSerializeSize(m.Nonce, nType, nVersion) +
        GetSerializeSize(m.NumFrames, nType, nVersion) +
        GetSerializeSize(m.NumUpdates, nType, nVersion) +
        GetSerializeSize(m.Updates[0], nType, nVersion)*m.NumUpdates;
}

template<typename Stream>
inline void Serialize(Stream& os, const MotoPoW& m, int nType, int nVersion)
{
    Serialize(os, m.Nonce, nType, nVersion);
    Serialize(os, m.NumFrames, nType, nVersion);
    Serialize(os, m.NumUpdates, nType, nVersion);
    for (uint32_t i = 0; i < m.NumUpdates; i++)
        Serialize(os, m.Updates[i], nType, nVersion);
}

template<typename Stream>
inline void Unserialize(Stream& is, MotoPoW& m, int nType, int nVersion)
{
    Unserialize(is, m.Nonce, nType, nVersion);
    Unserialize(is, m.NumFrames, nType, nVersion);
    Unserialize(is, m.NumUpdates, nType, nVersion);
    if (m.NumUpdates > MOTO_MAX_INPUTS)
        throw std::ios_base::failure("Unseralize nonce: NumUpdates > MOTO_MAX_INPUTS");

    for (uint32_t i = 0; i < m.NumUpdates; i++)
        Unserialize(is, m.Updates[i], nType, nVersion);
}
