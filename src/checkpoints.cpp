...
// Motocoin-FIXME: add checkpoints
static const bool g_MotocoinHasCheckpoints = false;
...
namespace Checkpoints
{
    ...
    // Motocoin-FIXME: add checkpoints (these are Litecoin checkpoints)
    ...
    {
        if (!g_MotocoinHasCheckpoints) return true;
    ...
    double GuessVerificationProgress(CBlockIndex *pindex) {
        if (pindex==NULL || !g_MotocoinHasCheckpoints)
            return 0.0;
    ...
    {
        if (!GetBoolArg("-checkpoints", true) || !g_MotocoinHasCheckpoints)
            return 0;
