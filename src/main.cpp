...
const string strMessageMagic = "Motocoin Signed Message:\n";
...
// 1-2 years: +10.0  mln per year (total: 20 mln)
// 3-4 years:  +5.0  mln per year (total: 30 mln)
// 5-6 years:  +2.5  mln per year (total: 35 mln)
// 7-8 years:  +1.25 mln per year (total: 37.5 mln)
// 9-10 years: +625k per year     (total: 38 750 000)
// 11-12 years: +312.5k per year  (total: 39 375 000)
// 13-forever years: +200k per year
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    // genesis blocks gives reward for each developer
    if (nHeight == 0)
...
/*
    uint32_t bnResult = nBase;
    while (nTime > 0 && bnResult < nProofOfWorkLimit)
    {
    ...
    if (bnResult > nProofOfWorkLimit)
        bnResult = nProofOfWorkLimit;
    return bnResult;*/
}
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *)
{
    ...
    if (pindexLast == NULL) {
      ...
      if (CurHeight % nInterval != 0) {
        ...
        // Go back the full period unless it's the first retarget after genesis. Code courtesy of Art Forz (who is awesome coder.)
        ...
        if(fTestNet) {
          sort(Times, Times+blockstogoback);
        }
...
bool IsInitialBlockDownload()
{
    ...
    // Time is increased to 72 hours for Motocoin.
    // During early life of Motocoin there may be not so many players,
    // so it is possible that no one will complete level for three days.
    return (GetTime() - nLastUpdate < 10 &&
            pindexBest->GetBlockTime() < GetTime() - 72 * 60 * 60);
}
...
bool CBlock::ConnectBlock(CValidationState &state, CBlockIndex* pindex, CCoinsViewCache &view, bool fJustCheck)
{
    ...
    // Litecoin: Special case for the genesis block, skipping connection of its transactions
    // Litecoin: (its coinbase is unspendable)
    // Motocoin: genesis coinbase IS spendable
    if (GetHash() == hashGenesisBlock) {
    ...
    if (pindex->GetUndoPos().IsNull() || (pindex->nStatus & BLOCK_VALID_MASK) < BLOCK_VALID_SCRIPTS)
    {
        if (pindex->GetUndoPos().IsNull()) {
            ...
            if (!blockundo.WriteToDisk(pos, (GetHash() == hashGenesisBlock)? uint256(0) : pindex->pprev->GetBlockHash()))
...
bool ProcessBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain)
    {
        ...
        uint32_t bnNewBlock = pblock->nBits;
        uint32_t bnRequired = ComputeMinWork(pcheckpoint->nBits, deltaTime);
...
static CBlock getGenesisBlock()
{
    ...
    const char* pszTimestamp = "Atletico Madrid win La Liga title in Barcelona 1:1 (Godin, Sanchez)";
    CTransaction txNew;
    txNew.vin.resize(1);
    txNew.vin[0].scriptSig = CScript() << nHeight << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));

    txNew.vout.resize(3);
    CBitcoinAddress Address;

    // Pay to first developer
    txNew.vout[0].nValue = 150000 * COIN;
    Address.SetString("MoSDFwjXGGjTGGmPgVDv3fw7ubp9wghprEr");
    txNew.vout[0].scriptPubKey.SetDestination(Address.Get());

    // Pay to second developer
    txNew.vout[1].nValue = 150000 * COIN;
    Address.SetString("ModQGjcaw1PNqfDxkxZjfdyoM3pdDH6T2g8");
    txNew.vout[1].scriptPubKey.SetDestination(Address.Get());

    // Pay to third developer (well, he is not really a developer, but he helped us a lot)
    txNew.vout[2].nValue = 150000 * COIN;
    Address.SetString("MoPMYMPPUwn4q9vHDxeAp5GvZkmwJWgPGLn");
    ...
    block.nVersion = 2;
    block.nTime    = 1400615000;
    block.nBits    = nProofOfWorkLimit;
    motoInitPoW(&block.Nonce);
...
bool LoadBlockIndex()
{
    ...
    hashGenesisMerkleRoot = getGenesisBlock().hashMerkleRoot;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc1, 0xb5, 0x9c };
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MotocoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MotocoinMiner : ProcessBlock, block not accepted");
    }
